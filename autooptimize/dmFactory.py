    ...
    def wheel(self, count, type='DOWN', minTime=0.01, maxtTime=0.1):
        if type == 'UP':
            for index in range(count):
                GlobalEnvStorage.infoLogger.info('¹ö¶¯Up %s %s', index, self.dm.WheelUp())
                time.sleep(uniform(minTime, maxtTime))

        else:
            for index in range(count):
                GlobalEnvStorage.infoLogger.info('¹ö¶¯down %s %s', index, self.dm.WheelDown())
                time.sleep(uniform(minTime, maxtTime))
